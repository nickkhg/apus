...
4. `wl_pointer`, so that an app reads the pointer. The shell keeps it now.
5. A shadow, a blur and a gradient in the display list, for the second mode of the design. `Appearance` holds the values of both modes already. See [toolkit.md](toolkit.md).
6. More than one desktop, and a layout for each one. The design has this as the target, and one desktop is what ships.
7. A picture of a window in the card that stands in for it. It is a crop of the top left at one pixel to one point. A person turns it on for one app at a time.
