...
`apus-gpu` does that with `LD_LIBRARY_PATH` and `MESA_LOADER_DRIVER_OVERRIDE`, and both of those cross `exec`. So the compositor takes them out of the environment it gives the apps it starts (`AppCatalog.environment`), and an app gets the drivers of the distribution. Without that step every app on the machine drew with the compositor's driver, and a program whose shaders do not all bind what they read — a browser, for one — read from a descriptor with nothing bound.
