...
Apus has four layers. Each layer uses the output of the layer before it.

| Layer | Tool | Input | Output | Documentation |
|---|---|---|---|---|
| Packages | `makepkg` | `packages/`, `ui/` | Local pacman repository `[apus]` | [packages.md](packages.md) |
...
| Installer | `apus-install` | The running live system | An installed disk | [system.md](system.md) |

The user interface is a fifth part. It is a set of Swift programs in the `apus-ui` package. See [ui.md](ui.md), [compositor.md](compositor.md), [toolkit.md](toolkit.md), and [applications.md](applications.md).
...
| `packages/` | Source of the Apus packages (one `PKGBUILD` in each directory). |
...
| `apus.xcodeproj`, `xcode/` | The Xcode project, and the script that its targets run. |
| `vm/` | `apus-vm`: the VM on the Mac, with Apple's Virtualization framework. |
