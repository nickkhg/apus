...
public enum SoftwareRenderer {
    ...
    private static func fill(_ path: Path, color: UInt32, clip: Rect, _ canvas: Canvas) {
        ...
        let top = max(box.y, Int(minimum.y.rounded(.down)))
        let bottom = min(box.y + box.height, Int(maximum.y.rounded(.up)) + 1)
        let left = max(box.x, Int(minimum.x.rounded(.down)))
        let right = min(box.x + box.width, Int(maximum.x.rounded(.up)) + 1)
