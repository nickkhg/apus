...
const uint16_t mydistro_ev_key = EV_KEY;
const uint16_t mydistro_ev_abs = EV_ABS;
const uint16_t mydistro_ev_syn = EV_SYN;
const uint16_t mydistro_syn_report = SYN_REPORT;
const uint16_t mydistro_abs_x = ABS_X;
const uint16_t mydistro_abs_y = ABS_Y;
const uint16_t mydistro_btn_left = BTN_LEFT;
// The same range as QEMU used for its tablet, so a test that worked in QEMU
// puts the pointer in the same place.
const int32_t mydistro_abs_maximum = 32767;

int mydistro_uinput_open(void) {
...
int mydistro_uinput_make_pointer(int fd) {
    ...
    axis.absinfo.maximum = mydistro_abs_maximum;
    axis.code = ABS_X;
    if (ioctl(fd, UI_ABS_SETUP, &axis) < 0) return -1;
    axis.code = ABS_Y;
    if (ioctl(fd, UI_ABS_SETUP, &axis) < 0) return -1;

    return finish(fd, "mydistro test pointer");
}

int mydistro_uinput_make_keyboard(int fd) {
    if (ioctl(fd, UI_SET_EVBIT, EV_KEY) < 0) return -1;
    ...
    return finish(fd, "mydistro test keyboard");
}

int mydistro_uinput_send(int fd, uint16_t type, uint16_t code, int32_t value) {
...
int mydistro_uinput_remove(int fd) {
